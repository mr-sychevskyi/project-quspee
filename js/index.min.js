...
var accordionBtn=_toConsumableArray(document.querySelectorAll('.accordion'));if(accordionBtn){accordionBtn.forEach(function(item){return item.addEventListener('click',function(e){e.preventDefault();var currItem=e.currentTarget;var currItemBtn=currItem.querySelector('.btn-accordion');var currItemPanel=currItem.querySelector('.accordion-panel');if(currItemPanel){currItemBtn.classList.toggle('open');currItemPanel.classList.toggle('d-block');}});});}

"use strict";$(function(){var form=$('.contact-form');var snackbar=$('.snackbar');$(form).submit(function(e){e.preventDefault();var formData=form.serialize();var formInput=form.find('.form-group__input');$.ajax({type:'POST',url:form.attr('action')||'./php/contact.php',data:formData,beforeSend:function beforeSend(data){form.find('button[type="submit"]').attr('disabled',true);},success:function success(data){if(data['error']){console.log(data['error']);}else{snackbar.addClass('show');formInput.val('');setTimeout(function(){toggleModal('contact-form');snackbar.removeClass('show');},5000);}},error:function error(xhr,ajaxOptions,thrownError){console.log(xhr.status);console.log(thrownError);},complete:function complete(data){form.find('button[type="submit"]').prop('disabled',false);}});});});

"use strict";function _toConsumableArray(arr){return _arrayWithoutHoles(arr)||_iterableToArray(arr)||_nonIterableSpread();}
function _nonIterableSpread(){throw new TypeError("Invalid attempt to spread non-iterable instance");}
function _iterableToArray(iter){if(Symbol.iterator in Object(iter)||Object.prototype.toString.call(iter)==="[object Arguments]")return Array.from(iter);}
function _arrayWithoutHoles(arr){if(Array.isArray(arr)){for(var i=0,arr2=new Array(arr.length);i<arr.length;i++){arr2[i]=arr[i];}return arr2;}}
var btnToggleItems=_toConsumableArray(document.querySelectorAll('.get-started-list .toggle-btn'));var serviceList=_toConsumableArray(document.querySelectorAll('.get-started-list .btn-service'));var billList=document.querySelector('.bill__list');var billHead=document.querySelector('.bill__head');var billTotal=document.querySelector('.bill__total');var billNotification=document.querySelector('.bill__notification');var btnModalToggle=document.querySelector('.btn-get-started');var billTotalPrice=document.querySelector('.bill__total-price');var getStartedSection=document.querySelector('.main__get-started');var contactModal=document.querySelector('.contact-modal');var contactModalBody=contactModal.querySelector('.services-choosed');var billSelected=getStartedSection.querySelector('.bill__list');var contactFormChoosed=contactModal.querySelector('.contact-form .contact-form__choosed');var contactFormTotal=contactModal.querySelector('.contact-form .contact-form__total');var btnModalClose=contactModal.querySelector('.btn-modal-close');var btnModalChange=contactModal.querySelector('.btn-change');var render=function render(serviceList){var selectedWorks=serviceList.filter(function(item){return item.classList.contains('selected');}).map(function(item){return{title:item.querySelector('.btn-service__title').innerText,price:99};});var total=selectedWorks.reduce(function(sum,curr){return sum+curr.price;},0);var hiddenFields=[billHead,billList,billTotal];billNotification.classList.remove('hidden');btnModalToggle.classList.add('disabled','pointer-none');hiddenFields.forEach(function(item){return item.classList.add('hidden');});if(!selectedWorks.length)return;billNotification.classList.add('hidden');btnModalToggle.classList.remove('disabled','pointer-none');hiddenFields.forEach(function(item){return item.classList.remove('hidden');});billList.innerHTML='';billTotalPrice.innerText="$ ".concat(total," / per month");selectedWorks.forEach(function(item){billList.innerHTML+="\n        <li class=\"bill__item\">\n            <span class=\"bill__description\">".concat(item.title,"</span>\n            <span class=\"bill__amount\">$ ").concat(item.price," / per month</span>\n        </li>\n        ");});};if(btnToggleItems){btnToggleItems.forEach(function(item){return item.addEventListener('click',function(e){e.stopPropagation();var currItem=e.currentTarget;var currItemBtn=currItem.closest?currItem.closest('.btn-service'):currItem.parentElement;e.target.type==='checkbox'?e.stopPropagation():currItemBtn.classList.toggle('selected');render(serviceList);});});}
var toggleModal=function toggleModal(){body.classList.toggle('scroll-state');modalOverlay.classList.toggle('closed');contactModal.classList.toggle('closed');};btnModalToggle.addEventListener('click',function(){contactModalBody.innerHTML=billSelected.outerHTML+billTotal.outerHTML;contactFormTotal.value=billTotalPrice.innerText;contactFormChoosed.value=_toConsumableArray(billSelected.querySelectorAll('.bill__description')).map(function(item){return item.innerText;}).join(' / ');toggleModal();});[btnModalClose,btnModalChange].forEach(function(item){return item.addEventListener('click',toggleModal);});modalOverlay.addEventListener('click',function(){body.classList.remove('scroll-state');modalOverlay.classList.add('closed');contactModal.classList.add('closed');workModal.classList.add('closed');});render(serviceList);

"use strict";function _toConsumableArray(arr){return _arrayWithoutHoles(arr)||_iterableToArray(arr)||_nonIterableSpread();}
function _nonIterableSpread(){throw new TypeError("Invalid attempt to spread non-iterable instance");}
function _iterableToArray(iter){if(Symbol.iterator in Object(iter)||Object.prototype.toString.call(iter)==="[object Arguments]")return Array.from(iter);}
function _arrayWithoutHoles(arr){if(Array.isArray(arr)){for(var i=0,arr2=new Array(arr.length);i<arr.length;i++){arr2[i]=arr[i];}return arr2;}}
var contactForm=document.querySelector('.contact-form');var contactFormBtn=contactForm.querySelector('.btn[type=submit]');if(contactFormBtn){contactFormBtn.addEventListener('click',function(e){var invalidFields=_toConsumableArray(contactForm.querySelectorAll(':invalid'));invalidFields.forEach(function(input){return input.classList.add('invalid');});contactForm.addEventListener('input',function(e){var currField=e.target;if(!currField.matches(':invalid')){currField.classList.remove('invalid');}});});}

"use strict";var countryGroupForm=contactForm.querySelector('.country-group');var contactFormInput=contactForm.querySelector('input[name$="country"]');if(contactFormInput){contactFormInput.addEventListener('input',function(e){var currValue=e.target.value;currValue.toUpperCase()==='USA'?countryGroupForm.classList.add('is-usa'):countryGroupForm.classList.remove('is-usa');});}

"use strict";function _toConsumableArray(arr){return _arrayWithoutHoles(arr)||_iterableToArray(arr)||_nonIterableSpread();}
function _nonIterableSpread(){throw new TypeError("Invalid attempt to spread non-iterable instance");}
function _iterableToArray(iter){if(Symbol.iterator in Object(iter)||Object.prototype.toString.call(iter)==="[object Arguments]")return Array.from(iter);}
function _arrayWithoutHoles(arr){if(Array.isArray(arr)){for(var i=0,arr2=new Array(arr.length);i<arr.length;i++){arr2[i]=arr[i];}return arr2;}}
var navbar=document.querySelector('.navbar');var navbarBrand=document.querySelector('.navbar__brand');var navbarToggle=document.querySelector('.btn-navbar-toggle');var navbarNavigation=document.querySelector('.navbar__navigation .main-navigation');var navigationMobileMenu=document.querySelector('.main-navigation-mobile');var navigationElements=_toConsumableArray(document.querySelectorAll('[data-nav]:not(.mobile-navigation-link)'));var heroOffset=document.querySelector('.hero-section .hero-content__item').offsetTop;var toggleInverseMode=function toggleInverseMode(foo){var action=foo?'add':'remove';navbar.classList[action]('navbar_inverse');navbarBrand.classList[action]('btn-brand_dark');navbarNavigation.classList[action]('main-navigation_inverse');};var toggleNavigationMenu=function toggleNavigationMenu(){body.classList.toggle('scroll-state');navbar.classList.toggle('navbar_transparent');navbarToggle.classList.toggle('active');navigationMobileMenu.classList.toggle('open');};navbarToggle.addEventListener('click',function(e){e.preventDefault();toggleNavigationMenu();});navigationMobileMenu.addEventListener('click',function(e){var currItem=e.target;var currItemData=currItem.getAttribute('data-nav');if(currItem.classList.contains('mobile-navigation-link')){toggleNavigationMenu();setTimeout(function(){document.querySelector("#".concat(currItemData)).scrollIntoView({block:"start",behavior:'smooth'});},250);}});navigationElements.forEach(function(item){return item.addEventListener('click',function(e){e.stopPropagation();var currItem=e.currentTarget;var currItemData=currItem.getAttribute('data-nav');document.querySelector("#".concat(currItemData)).scrollIntoView({block:"start",behavior:'smooth'});});});var toggleNavbar=function toggleNavbar(){return toggleInverseMode(window.scrollY>heroOffset-50);};window.addEventListener('load',toggleNavbar,false);window.addEventListener('scroll',toggleNavbar,false);

"use strict";$(document).ready(function(){var swiper=new Swiper('.swiper-container',{slidesPerView:3,slidesPerColumn:2,spaceBetween:40,navigation:{nextEl:'.swiper-button-next',prevEl:'.swiper-button-prev'},breakpoints:{480:{slidesPerView:1,slidesPerColumn:3},768:{slidesPerView:2}}});});

"use strict";var tasksValue=document.querySelector('.tasks .tasks-done');var startDate=new Date("Dec 01, 2018 00:00:00").getTime();var now=new Date().getTime();var timePassed=Math.abs(now-startDate);var hoursPassed=Math.floor(timePassed/(1000*60*60));if(tasksValue){tasksValue.innerText=1029+Math.floor(hoursPassed/5);}
